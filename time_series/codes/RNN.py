...
class RNNModel(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.rnn = nn.RNN(input_size=self.channels, hidden_size=self.hidden_dim, batch_first=True)
    ...
    def forward(self, x):
        ...
        out, _ = self.rnn(x, h0)  # RNN computation
        
        if self.individual:
            output = torch.zeros([x.size(0), self.pred_len, x.size(2)], dtype=x.dtype).to(x.device)
            for i in range(self.channels):
                output[:, :, i] = self.Linear[i](out[:, -1, :])  # Using last RNN output
            x = output
        else:
